...
subtest 'count FOO' => sub {
    plan tests => 1;
    my $R = $PT->request( GET('/count') );
    ok $R->is_success;
};
...
subtest 'info FOO after' => sub {
    ...
    is $after->{ds}{foo}{last_ds} => 'U';
    is $after->{ds}{foo}{value}   => 0;
};

subtest 'commit FOO' => sub {
    plan tests => 1;
    my $R = $PT->request( GET('/commit') );
    ok $R->is_success;
};
